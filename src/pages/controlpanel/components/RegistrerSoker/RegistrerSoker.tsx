  ...
  return (
    <section className="flex flex-col items-center w-screen h-screen bg-vektor-bg">
      ...
      <div className="flex rounded-lg bg-vektor-kontrollblue w-1/2 p-5 justify-between my-5 text-table-grey">
        ...
        <div className="">
          <button type="button" onClick={removeBox} className="">x</button>
        </div>
      </div>
      )}
      <h1 className="w-1/2 text-5xl text-center my-5 max-md:text-3xl">Registrer søker NTNU Høst 2023</h1>
      <form onSubmit={submitForm} className="flex flex-col justify-center items-center w-1/2 gap-10">
        <div className="w-full max-w-[300px]">
          <label className="flex flex-col label w-full" htmlFor="input-firstname">
            <p className="w-full">Fornavn</p>
            <input className="w-full input input-info" name="firstname" id="input-firstname" type="text" placeholder="" required />
          </label>
        </div>
        <div className="w-full max-w-[300px]">
          <label className="flex flex-col label w-full" htmlFor="input-lastname">
            <p className="w-full">Etternavn</p>
            <input className="w-full input input-info w-full" name="lastname" id="input-lastname" type="text" placeholder="" required />
          </label>
        </div>
        <div className="w-full max-w-[300px]">
          <label className="flex flex-col label w-full" htmlFor="input-phone">
            <p className="w-full">Telefon</p>
            <input className="w-full inline input input-info" name="phone" id="input-phone" type="phone" placeholder="" required />
          </label>
        </div>
        <div className="w-full max-w-[300px]">
          <label className="flex flex-col label w-full" htmlFor="input-email">
            <p className="w-full">Email</p>
            <input className="w-full input input-info w-full" name="email" id="input-email" type="email" placeholder="" required />
          </label>
        </div>
